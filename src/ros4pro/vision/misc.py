...
def load_mnist_data(kept_labels=[1, 2], batch_size=128, root='./data'):
    """
    ...
    train_idx = [i for i in range(len(trainset)) if trainset.targets[i] in kept_labels]
    trainset.data = trainset.data[train_idx]
    trainset.targets = trainset.targets[train_idx]
    test_idx = [i for i in range(len(testset)) if testset.targets[i] in kept_labels]
